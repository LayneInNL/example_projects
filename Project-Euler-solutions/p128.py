...
import eulerlib.eulerlib, itertools
...
def compute():
	...
	for ring in itertools.count(2):
		if all(map(eulerlib.eulerlib.is_prime, (ring * 6 - 1, ring * 6 + 1, ring * 12 + 5))):
			count += 1
			if count == TARGET:
				return str(ring * (ring - 1) * 3 + 2)
		if all(map(eulerlib.eulerlib.is_prime, (ring * 6 - 1, ring * 6 + 5, ring * 12 - 7))):
			count += 1
			if count == TARGET:
