...
def compute():
	isprime = eulerlib.eulerlib.list_primality(10000)  # Cache for small numbers
	...
	def is_prime(n):
		if n < len(isprime):
			...
			return eulerlib.eulerlib.is_prime(n)
	
	ans = 0
	while True:
		ans += count_prime_sets(0, 0)
		if not eulerlib.eulerlib.next_permutation(digits):
			break
	return str(ans)
