...
import eulerlib.eulerlib


def compute():
	isprime = eulerlib.eulerlib.list_primality(20000000)
...
def sam_transitions_minus_max_transitions(n):
	...
	while True:
		...
		if newstate == segmentstate:
			break
		maxtrans += eulerlib.eulerlib.popcount(newstate ^ segmentstate)
		segmentstate = newstate
		samtrans += 2 * eulerlib.eulerlib.popcount(newstate)
		n = digit_sum(n)
	maxtrans += eulerlib.eulerlib.popcount(segmentstate)
