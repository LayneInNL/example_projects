...
import eulerlib.eulerlib, math


def compute():
	ans = sum(1 for i in range(1, 10001) if (not eulerlib.eulerlib.is_square(i) and get_sqrt_continued_fraction_period(i) % 2 == 1))
	return str(ans)
...
class QuadraticSurd:
	...
	def floor(self):
		temp = eulerlib.eulerlib.sqrt(self.b * self.b * self.d)
		if self.b < 0:
			temp = -(temp + 1)
		temp += self.a
		if temp < 0:
